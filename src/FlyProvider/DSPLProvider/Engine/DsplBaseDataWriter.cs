...
namespace DsplDataFormat.Engine
{
    public class DsplBaseDataWriter : IDsplDataWriterEngine, IDisposable
    {
        ...
        private List<DictionaryEntry> appsettings = null;

        public List<DictionaryEntry> Appsettings
        {
            get
            {
                if (appsettings == null)
                {
                    appsettings = new List<DictionaryEntry>()
            {
                new DictionaryEntry("urlAddress" , "http:\\www.istat.it"),
                new DictionaryEntry("UriEntity" , "http://www.google.com/publicdata/dataset/google/entity"),
                new DictionaryEntry("UriMetrics" , "http://www.google.com/publicdata/dataset/metrics"),
                new DictionaryEntry("UriQuantity" , "http://www.google.com/publicdata/dataset/google/quantity"),
                new DictionaryEntry("UriGeo" , "http://www.google.com/publicdata/dataset/google/geo"),
                new DictionaryEntry("UriTime" , "http://www.google.com/publicdata/dataset/google/time"),
                new DictionaryEntry("DsplSchema" , "http://schemas.google.com/dspl/2010"),
            };
                }
                return appsettings;
            }
        ...
        {
            DictionaryEntry val = Appsettings.Find(a => a.Key.ToString().Trim().ToUpper() == key.Trim().ToUpper()); ;
            if (val.Value != null)
        ...
        public void CloseAndZip(StreamWriter tw)
        {
            File.WriteAllText(dirPath + "\\" + "DATI.csv", csvdati.ToString());
            csvdati = null;
            csvdati = new StringBuilder();
            ...
            this._dsplDataWriter.CompressDirectory(directory, directorySelected);
            System.Web.HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=" + pasta[total - 1] + ".zip");

            using (StreamReader rea = new StreamReader(dirPath + ".zip"))
            {
                rea.BaseStream.CopyTo(tw.BaseStream);
            }
        }
        ...
        public void ClearTempFolder()
        {
            try
            {
                DirectoryInfo directorySelected = new DirectoryInfo(dirPath);
                string[] pasta = dirPath.Split('\\');
                DirectoryInfo directory = new DirectoryInfo(dirPath.Replace("\\" + pasta[pasta.Length - 1], ""));
                this._dsplDataWriter.CleanDirectory(directorySelected);
                this._dsplDataWriter.DeleteDirectory(directory, directorySelected);
                dirPath = "";
            ...
            {
                //COme se non fosse successo nulla rimarrà sporco...
            }
